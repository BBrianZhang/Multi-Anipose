...
def visualize_labels(config, labels_fname, vid_fname, outname):
    ...
    if len(scheme) == 0:
        bodyparts = list(dlabs.columns.levels[1])
    else:
        bodyparts = sorted(set([x for dx in scheme for x in dx]))

    individuals = list(dlabs.columns.levels[0])
    ...
    for i in range(len(individuals)):
        point = [(dlabs[individuals[i]][bp]['x'], dlabs[individuals[i]][bp]['y']) for bp in bodyparts]
        point = np.array(point)

        score = [dlabs[individuals[i]][bp]['likelihood'] for bp in bodyparts]
        score = np.array(score)
        score[np.isnan(score)] = 0
        score[np.isnan(point[:, 0])] = 0

        good = np.array(score) > 0.1
        point[:, 0, :][~good] = np.nan
        point[:, 1, :][~good] = np.nan
    ...
    for ix in trange(last, ncols=70):
        ...
        for i in range(len(individuals)-1):
            point = all_points[i][:, :, ix]
            img = label_frame(img, point, scheme, bodyparts)

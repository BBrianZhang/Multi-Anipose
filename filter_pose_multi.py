...
def load_pose_2d(fname):
    ...
    id_index = data.columns.names.index('individuals')
    ...
    individuals = list(data.columns.get_level_values(id_index).unique())
    n_possible = len(data.columns.levels[coord_index])//3

    n_frames = len(data)
    n_joints = len(bodyparts) * len(individuals)
    new_data = np.array(data)[:, 0:n_joints * 3]
    test = np.array(new_data).reshape(n_frames, n_joints, n_possible, 3)
...
def write_pose_2d(all_points, metadata, outname=None):
    ...
    dout.loc[:, (scorer, individuals, bodyparts, 'x')] = points[:, :, 0]
    dout.loc[:, (scorer, individuals, bodyparts, 'y')] = points[:, :, 1]
    dout.loc[:, (scorer, individuals, bodyparts, 'likelihood')] = scores
...
def filter_pose_medfilt(config, all_points, bodyparts, individuals):
    n_frames, n_joints, n_possible, _ = all_points.shape
    ...
    for bp_ix, bp in enumerate(bodyparts):
        for id_ix, id in enumerate(individuals):
            x = points_full[:, bp_ix+id_ix*len(bodyparts), 0, 0]
            y = points_full[:, bp_ix+id_ix*len(bodyparts), 0, 1]
            score = scores_full[:, bp_ix+id_ix*len(bodyparts), 0]
            ...
            points[:, bp_ix+id_ix*len(bodyparts), 0] = Xfi[:, 0]
            points[:, bp_ix+id_ix*len(bodyparts), 1] = Xfi[:, 1]
...
def process_session(config, session_path):
    ...
    for fname in pose_files:
        ...
        # print(metadata, all_points)

        for filter_type in filter_types:
            filter_fun = FILTER_MAPPING[filter_type]
            points, scores = filter_fun(config, all_points, metadata['bodyparts'], metadata['individuals'])

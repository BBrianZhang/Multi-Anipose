...
from .triangulate_multi import load_offsets_dict

from .label_videos_multi import label_frame
...
def get_projected_points(config, pose_fname, cgroup, offsets_dict):
    ...
    individuals = [id.replace('_' + bodyparts[0] + '_error', '') for id in
                   [x for x in cols if '_' + bodyparts[0] + '_error' in x]]
    ...
    for id in individuals:
        all_points = np.array([np.array(pose_data.loc[:, (id + '_' + bp + '_x', id + '_' + bp + '_y', id + '_' + bp + '_z')])
                               for bp in bodyparts], dtype='float64')
        all_ind_points.append(all_points)

        all_errors = np.array([np.array(pose_data.loc[:, id + '_' + bp + '_error'])
                               for bp in bodyparts], dtype='float64')
    ...
    n_joints, n_frames, _ = all_ind_points[0].shape
    ...
    all_points_flat_t = [(all_points_flat[id] + center).dot(np.linalg.inv(M.T)) for id in range(len(individuals))]
    ...
    points_2d_proj_flat = [cgroup.project(all_points_flat_t[id]) for id in range(len(individuals))]
    points_2d_proj = [points_2d_proj_flat[id].reshape(n_cams, n_joints, n_frames, 2) for id in range(len(individuals))]
    ...
    for cix, cname in enumerate(cam_names):
        ...
        for id in range(len(individuals)):
            points_2d_proj[id][cix, :, :, 0] -= dx
            points_2d_proj[id][cix, :, :, 1] -= dy

    return scheme, bodyparts, individuals, points_2d_proj
...
def visualize_combined(config, pose_fname, cgroup, offsets_dict,
    ...
    if should_load_3d:
        scheme, bodyparts, individuals, points_2d_proj = get_projected_points(config, pose_fname, cgroup, offsets_dict)
        ...
        if should_load_3d:
            for id in range(len(individuals)):
                frames_2d = draw_projected_points(
                    frames_2d, scheme, bodyparts, points_2d_proj[id][:, :, framenum])
